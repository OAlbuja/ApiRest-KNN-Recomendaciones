...
public class KNNService implements IKNNService{
    ...
    public List<ClienteDistanciaDTO> encontrarVecinosCercanos(Long clienteId, int k) {
        ...
        List<ClienteDistanciaDTO> distancias = todosLosClienteIds.stream()
                .map(id -> {
                    List<ClienteInteresDTO> interesesClienteComparado = clienteInteresService
                            .findInteresesByClienteId(id);
                    double[] interesesArrayClienteComparado = convertirInteresesADoubleArray(interesesClienteComparado);
                    double distancia = calcularDistanciaEuclidiana(interesesArrayClienteObjetivo,
                            interesesArrayClienteComparado);
    ...
    private double[] convertirInteresesADoubleArray(List<ClienteInteresDTO> interesesDTO) {
        ...
        double[] interesesArray = new double[(int) totalIntereses]; // Se asume que totalIntereses es siempre un valor
                                                                    // razonable para convertir a int
